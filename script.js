...
function populateModelSelectFromLocalData() {
    if (modeloElement && window.carros && window.carros.length > 0) {
        ...
        console.warn(
            "Elemento modelo ou dados de carros locais não disponíveis para popular o select."
        );
    }
}
...
async function updateCarPrice() {
    ...
    if (selectedCar && selectedCar.fipe) {
        ...
        if (priceHistory) {
            if (priceHistory.length >= 3) {
                const currentPrice = parseCurrencyToFloat(
                    priceHistory[0].price
                );
                const threeMonthsAgoPrice = parseCurrencyToFloat(
                ...
                DEPRECIACAO_ANUAL_RATE_DEFAULT =
                    (priceChange / threeMonthsAgoPrice) * (12 / 3); // Projeta para um ano
                ...
                console.log(
                    `Nova taxa de depreciação do primeiro ano calculada: ${(
                        DEPRECIACAO_ANUAL_RATE_DEFAULT * 100
                    ).toFixed(2)}%`
                );
            } else {
                console.log(
                    "Dados insuficientes para calcular a depreciação dinâmica. Usando a taxa padrão de 15%.",
                    priceHistory[0].price
                );
                const currentPrice = parseCurrencyToFloat(
                    priceHistory[0].price
            ...
            console.warn(
                "Não há dados suficientes da FIPE para calcular a depreciação dinâmica. Usando a taxa padrão de 15%."
            );
        }
    } else {
        DEPRECIACAO_ANUAL_RATE_DEFAULT = 0.15;
    ...
    console.log(
        `Preço do carro "${
            selectedCar?.modelo || "Nenhum"
        }" e taxa de depreciação atualizados.`
    );
}
...
function updateCarData() {
    ...
    if (selectedCar && selectedCar[keyManutencao]) {
        manutencaoElement.value = formatCurrency(
            selectedCar[keyManutencao] / 12
        );
    } else {
    ...
    console.log(
        `Dados do carro "${
            selectedCar?.modelo || "Nenhum"
        }" atualizados sem nova requisição FIPE.`
    );
}

const irAliquots = [
    { months: 6, aliquot: 0.225 },
    { months: 12, aliquot: 0.2 },
    { months: 24, aliquot: 0.175 },
...
function createJurosCurveTable() {
    ...
    if (!anbimaData || Object.keys(anbimaData).length === 0) {
        console.warn(
            "Dados da ANBIMA não carregados para criar a curva de juros."
        );
        return [];
    }
    ...
    for (let month = 1; month <= 36; month++) {
        ...
        if (periodYears > 0) {
            const term1 =
                (1 - Math.exp(-lambda1 * periodYears)) /
                (lambda1 * periodYears);
            const term2 = term1 - Math.exp(-lambda1 * periodYears);
            const term3 =
                (1 - Math.exp(-lambda2 * periodYears)) /
                (lambda2 * periodYears);
        ...
        const taxaLiquidaAoPeriodo =
            (1 - irAliquot) * (taxaBrutaAoPeriodo - custoCorretagemMensal);
        jurosCurve.push({
            Mês: month,
            "Taxa Bruta ao ano": `${(yieldAnnual * 100).toLocaleString(
                "pt-BR",
                {
            ...
            "Taxa Bruta ao período": `${(
                taxaBrutaAoPeriodo * 100
            ).toLocaleString("pt-BR", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 4,
            })}%`,
            "Taxa Líquida ao período": `${(
                taxaLiquidaAoPeriodo * 100
            ).toLocaleString("pt-BR", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 4,
            })}%`,
...
$(document).ready(function () {
    ...
    custoOportunidadeFinanciadaTotalElement = $(
        '[data-total="custo_oportunidade_financiada"]'
    );
    custoOportunidadeVistaTotalElement = $(
        '[data-total="custo_oportunidade_vista"]'
    );
    ...
    custoOportunidadeAssinaturaTotalElement = $(
        '[data-total="custo_oportunidade_assinatura"]'
    );
...
function parsePercentageToFloat(percentageString) {
    ...
    return parseFloat(
        percentageString.replace("%", "").replace(",", ".").trim()
    );
}
...
function calculateOpportunityCost(
    ...
    anbimaData,
    allCalculatedValues,
    annualFinancialDetails
) {
    let totalOpportunityCost = 0;
    const jurosCurve = createJurosCurveTable();
    if (
        !anbimaData ||
        Object.keys(anbimaData).length === 0 ||
        period <= 0 ||
        !jurosCurve ||
        jurosCurve.length === 0
    ) {
        console.warn(
            "Dados insuficientes para calcular Custo de Oportunidade."
        );
        return 0;
    }
    const {
    ...
    for (let month = 1; month <= period; month++) {
        ...
        const remainingPeriodLiquidRateData = jurosCurve.find(
            (item) => item["Mês"] === remainingPeriod
        );
        if (!remainingPeriodLiquidRateData) {
            console.warn(
                `Taxa Líquida ao período não encontrada para o período restante (${remainingPeriod} meses) no mês ${month}.`
            );
            continue;
        }
        const remainingPeriodLiquidRate =
            parsePercentageToFloat(
                remainingPeriodLiquidRateData["Taxa Líquida ao período"]
            ) / 100;
        ...
        if (scenarioType === "vista") {
            ...
            if (firstMonthOfYear && annualFinancialDetails[currentYearIndex]) {
                cashOutflowThisMonth +=
                    annualFinancialDetails[currentYearIndex].annualSeguro;
                cashOutflowThisMonth +=
                    annualFinancialDetails[currentYearIndex].annualIpva;
                cashOutflowThisMonth +=
                    annualFinancialDetails[currentYearIndex]
                        .annualLicenciamento;
            }
        ...
        const monthlyOpportunityCost =
            cashOutflowThisMonth * remainingPeriodLiquidRate;
...
function onFormChange() {
    ...
    for (let year = 0; year < fullYearsInPeriod; year++) {
        const annualSeguroThisYear =
            (seguroPercentage * currentCarValueForDepreciation) / 100;
        const annualIpvaThisYear =
            (ipvaPercentage * currentCarValueForDepreciation) / 100;
        ...
        const depreciationRateForThisYear =
            year === 0
                ? DEPRECIACAO_ANUAL_RATE_DEFAULT
                : DEPRECIACAO_ANUAL_RATE_LONGER_PERIOD;
        const depreciationAmountThisYear =
            currentCarValueForDepreciation * depreciationRateForThisYear;
        ...
        console.log(
            `Fim do Ano ${year + 1}: Valor do carro = ${formatCurrency(
                currentCarValueForDepreciation
            )}`
        );
    }
    ...
    if (remainingMonthsInPartialYear > 0) {
        ...
        const annualSeguroThisPartialYear =
            (seguroPercentage * currentCarValueForDepreciation) / 100;
        const annualIpvaThisPartialYear =
            (ipvaPercentage * currentCarValueForDepreciation) / 100;
        ...
        totalLicenciamentoPeriodo +=
            annualLicenciamentoThisPartialYear * monthsFraction;

        const depreciationRateForPartialYear =
            fullYearsInPeriod === 0
                ? DEPRECIACAO_ANUAL_RATE_DEFAULT
                : DEPRECIACAO_ANUAL_RATE_LONGER_PERIOD;
        const depreciationAmountThisPartialYear =
            currentCarValueForDepreciation *
            depreciationRateForPartialYear *
            monthsFraction;
        ...
        console.log(
            `Fim do Período (após ${periodo} meses): Valor do carro = ${formatCurrency(
                currentCarValueForDepreciation
            )}`
        );
    ...
    if (taxaMensalDecimal > 0) {
        parcelaMensal =
            (valorFinanciado *
                (taxaMensalDecimal *
                    Math.pow(1 + taxaMensalDecimal, periodo))) /
            (Math.pow(1 + taxaMensalDecimal, periodo) - 1);
    } else {
    ...
    const custoOportunidadeFinanciada = calculateOpportunityCost(
        "financiada",
        entradaTotal,
        periodo,
        anbimaData,
        allCalculatedValuesForOpportunityCost,
        annualFinancialDetails
    );
    const custoOportunidadeVista = calculateOpportunityCost(
        "vista",
        preco,
        periodo,
        anbimaData,
    ...
    const custoOportunidadeAssinatura = calculateOpportunityCost(
        "assinatura",
        null,
        periodo,
        anbimaData,
    ...
    custoOportunidadeFinanciadaTotalElement.text(
        formatCurrency(custoOportunidadeFinanciada)
    );
    custoOportunidadeVistaTotalElement.text(
        formatCurrency(custoOportunidadeVista)
    );
    custoOportunidadeAssinaturaTotalElement.text(
        formatCurrency(custoOportunidadeAssinatura)
    );

    const financiadaCalcTotal =
        seguroTotal +
        ipvaTotal +
        manutencaoTotal +
        totalLicenciamentoPeriodo +
        emplacamentoValue +
        jurosTotal +
        custoOportunidadeFinanciada +
        totalDepreciacaoCalculated;
    financiadaTotalElement.text(formatCurrency(financiadaCalcTotal));
    const vistaCalcTotal =
        seguroTotal +
        ipvaTotal +
        manutencaoTotal +
        totalLicenciamentoPeriodo +
        emplacamentoValue +
        custoOportunidadeVista +
        totalDepreciacaoCalculated;
    vistaTotalElement.text(formatCurrency(vistaCalcTotal));
    assinaturaTotalElement.text(
        formatCurrency(assinaturaTotal + custoOportunidadeAssinatura)
    );

    if (anbimaData && Object.keys(anbimaData).length > 0) {
        const formattedAnbima = `beta1: ${anbimaData.beta1
            .toFixed(4)
            .replace(".", ",")} | beta2: ${anbimaData.beta2
            .toFixed(4)
            .replace(".", ",")} | beta3: ${anbimaData.beta3
            .toFixed(4)
            .replace(".", ",")} | beta4: ${anbimaData.beta4
            .toFixed(4)
            .replace(".", ",")} | lambda1: ${anbimaData.lambda1
            .toFixed(4)
            .replace(".", ",")} | lambda2: ${anbimaData.lambda2
            .toFixed(4)
            .replace(".", ",")}`;
    ...
    console.log(
        `Custo de Oportunidade Financiada: ${custoOportunidadeFinanciada}`
    );
    console.log(`Custo de Oportunidade à Vista: ${custoOportunidadeVista}`);
    console.log(
        `Custo Rentabilidade Assinatura: ${custoOportunidadeAssinatura}`
    );
